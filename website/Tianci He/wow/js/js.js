...
function Hlink(initComponent){
    this.textNode = initComponent.content;
...
function addText_H(header){
    ...
    var boldll = document.createElement('strong');
    var text  = document.createTextNode(header.toString());
    boldll.appendChild(text);
    temp.appendChild(boldll);
    mainContent.appendChild(temp);
}
...
function loadSlideshow(slideShow) {
	...
	var rawSlide = slideShow.slides[i];
	var slide = new Slide(rawSlide.imgFile, rawSlide.caption);
	slides[i] = slide;
    }
    if (slides.length > 0)
	currentSlide = 0;
